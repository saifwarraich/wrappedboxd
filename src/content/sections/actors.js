...
function renderPersonGrid(people, allFilms, container, entityType) {
  ...
  function render() {
    ...
    container.innerHTML = `
      ...
      <div class="lbs-person-grid" id="lbs-${entityType}-grid">${renderGrid()}</div>
      ${hasMore ? `
    ...
    container.querySelectorAll('.lbs-person-card').forEach(card => {
      card.addEventListener('click', () => {
        ...
        const films = allFilms.filter(f => f.cast?.some(c => c.name === name));
...
export function renderActors(films, container) {
  ...
  renderPersonGrid(actors, films, container, 'actors');
}
